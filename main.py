...
def index():
   ...
   #Getting a single recipe for now 
    # removeOld = db.select(model.Recipe).where(model.Recipe.id == 123)
    removeOld = db.session.get(model.Recipe,123)
    db.session.delete(removeOld)
    db.session.commit()
    ...
    testRecipe = model.Recipe(
        ...
        description = "This is a really good recipe guys I swear I didn't just make this up please belive me",
        user_id = 3,
        servings = 0,
        time = 140
    )
    ...
    return render_template("main/index.html", posts=posts, recipes=recipes)
